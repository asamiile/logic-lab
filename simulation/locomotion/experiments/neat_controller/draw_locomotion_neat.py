...
def get_args():
    parser = argparse.ArgumentParser(description='Visualize NEAT controller')
    parser.add_argument(
        '-e', '--experiment',
...
def load_best_genome(experiment_dir):
    ...
    # Get all genome files and sort by generation (earlier generations have lower IDs)
...
def visualize_controller(genome, task, episodes=3, max_steps=1000, render_mode='rgb_array'):
    ...
    # Load configuration
    config_file = os.path.join(CURR_DIR, 'config', 'locomotion_neat.cfg')
    ...
    config_content = config_content.replace('num_inputs              = 1', f'num_inputs              = {obs_dim}')
    config_content = config_content.replace('num_outputs             = 1', f'num_outputs             = {act_dim}')

    temp_config_file = os.path.join(CURR_DIR, 'config', 'locomotion_neat_draw.cfg')
    with open(temp_config_file, 'w') as f:
        f.write(config_content)

    config = neat_cppn.make_config(temp_config_file)
