...
def get_args():
    parser = argparse.ArgumentParser(description="Evogym Hyper-NEAT experiment")
    parser.add_argument(
        "-n", "--name", default="", type=str, help='experiment name (default: "{task}_{robot}")'
    )
    parser.add_argument(
        "-t",
        "--task",
    ...
    parser.add_argument(
        "-p", "--pop-size", default=200, type=int, help="population size of NEAT (default: 200)"
    )
    parser.add_argument(
        "-g", "--generation", default=500, type=int, help="iterations of NEAT (default: 500)"
    )
    parser.add_argument(
        "--use-hidden",
        action="store_true",
    ...
    assert args.task in BASELINE_ENV_NAMES, (
        f'argumented task id "{args.task}" is not prepared, pick from ['
        + ", ".join(BASELINE_ENV_NAMES)
        + "]."
    )
...
def get_figure_args():
    parser = argparse.ArgumentParser(description="make robot figures")
    ...
    parser.add_argument(
        "--start-timestep", type=int, default=0, help="start timestep of render (default: 0)"
    )
    parser.add_argument(
        "-ti",
        "--timestep-interval",
