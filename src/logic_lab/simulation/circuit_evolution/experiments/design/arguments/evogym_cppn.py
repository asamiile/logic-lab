...
def get_args():
    parser = argparse.ArgumentParser(description="Evogym CPPN experiment")

    parser.add_argument(
        "-n", "--name", default="", type=str, help='experiment name (default: "{task}")'
    )
    parser.add_argument(
        "-t",
        "--task",
    ...
    parser.add_argument(
        "-p", "--pop-size", default=4, type=int, help="population size of NEAT (default: 4)"
    )
    parser.add_argument(
    ...
    assert args.task in BASELINE_ENV_NAMES, (
        f'argumented task id "{args.task}" is not prepared, so pick from ['
