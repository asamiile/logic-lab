...
class MazeReporterNEAT(BaseReporterNEAT):
    ...
    def post_evaluate(self, config, population, species, best_genome):
        ...
        current_points = self.ax.scatter(
            current_data[:, 0], current_data[:, 1], s=3.0, color=[1.0, 0.4, 0.2], alpha=0.9
        )

        start_point = self.ax.scatter(
            self.start_point[0], self.start_point[1], color=[0.0, 0.6, 0.3], s=96, marker="s"
        )
        goal_point = self.ax.scatter(
            self.exit_point[0], self.exit_point[1], color=[0.9, 0.2, 0.0], s=128, marker="*"
        )

        line_best = self.ax.plot(
            self.best_path[:, 0], self.best_path[:, 1], linewidth=3, c="b", alpha=0.7
        )
...
class MazeReporterNS(BaseReporterNS):
    ...
    def post_evaluate(self, config, population, species, best_genome):
        ...
        line_best = self.ax.plot(
            self.best_path[:, 0], self.best_path[:, 1], linewidth=3, c="b", alpha=0.7, label="best"
        )
        line_novelty = self.ax.plot(
            self.novelty_path[:, 0],
            self.novelty_path[:, 1],
            linewidth=3,
            c="orange",
        ...
        legend = plt.legend(
            bbox_to_anchor=(0.95, 0.97), loc="lower right", borderaxespad=0, fontsize=10
        )
