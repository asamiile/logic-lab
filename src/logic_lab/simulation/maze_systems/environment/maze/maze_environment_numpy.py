...
class Agent:
    ...
    def update_rangefinder_sensors(self, walls):
        ...
        finder_points = (
            ...
            * np.vstack([np.cos(range_finder_angles), np.sin(range_finder_angles)]).T
        )
        ...
        distances = np.where(
            (Bot != 0) & (r > 0) & (r < 1) & (s > 0) & (s < 1),
            np.linalg.norm(A + np.expand_dims(r, axis=-1) * BA - C, axis=-1),
            self.range_finder_range,
        )
    ...
    def update_radars(self, exit_point):
        exit_angle = (
            np.arctan2(exit_point[0] - self.location[0], exit_point[1] - self.location[1]) % np.pi
        )
...
class MazeEnvironment:
    def __init__(
        self, init_location, walls, exit_point, init_heading=180, exit_range=5.0, agent_kwargs={}
    ):
        self.walls = walls
    ...
    def reset(self):
        self.agent = Agent(
            location=self.init_location, heading=self.init_heading, **self.agent_kwargs
        )
    ...
    def test_wall_collision(self, location):
        ...
        dist1 = np.minimum(np.linalg.norm(A - C, axis=1), np.linalg.norm(B - C, axis=1))
    ...
    def update(self, control_signals):
        ...
        vel = np.array(
            [
                np.cos(self.agent.heading / 180 * np.pi) * self.agent.speed,
    ...
    def read_environment(ROOT_DIR, maze_name, maze_kwargs={}, agent_kwargs={}):
        ...
        """
        maze_file = os.path.join(ROOT_DIR, "environment", "maze", "maze_files", f"{maze_name}.txt")
