...
class WallGene:
    ...
    def mutate(self, config):
        ...
        self.passage_location = max(
            0, min(0.9999, self.passage_location + random.uniform(-scale, scale))
        )
        self.horizontal = bool(random.getrandbits(1))
...
class MazeGenomeConfig:
    ...
    def __init__(self, params):
        ...
        rad = 2 * rng + 1
        self.path_shift_candidates = [
            (i // rad - rng, i % rad - rng) for i in range(rad**2) if i != (2 * rad + rng)
        ]
...
class MazeGenome:
    ...
    def mutate(self, config):
        if config.single_structural_mutation:
            ...
            while not valid:
                ...
                assert (
                    mutate_func is not None
                ), f"{mutate_func_name} is not defined in MazeGenome class"
    ...
    def mutate_path_attr(self, config):
        ...
        while not valid and invalid_count < 50:
            path_attrs = [
                (path_gene.pathpoint, path_gene.horizontal) for path_gene in self.path_genes
            ]
    ...
    def get_juncture_num(self):
        return len(self.path_genes) + len(
            [
                1
                for i in range(len(self.path_genes) - 1)
                if self.path_genes[i].horizontal == self.path_genes[i + 1].horizontal
            ]
        )
    ...
    def get_path_length(self):
        return (
            sum(
                [
                    abs(self.path_genes[i + 1].pathpoint[0] - self.path_genes[i].pathpoint[0])
                    + abs(self.path_genes[i + 1].pathpoint[1] - self.path_genes[i].pathpoint[1])
                    for i in range(len(self.path_genes) - 1)
                ]
            )
            + abs(self.maze_size[0] - 1 - self.path_genes[-1].pathpoint[0])
            + abs(self.maze_size[1] - 1 - self.path_genes[-1].pathpoint[1])
        )
    ...
    def check_path_validity(pathways, maze_size):
        ...
        for pathway in pathways[::-1]:
            ...
            if (
                cur_p[0] < 0
                or cur_p[1] < 0
                or cur_p[0] >= maze_size[0]
                or cur_p[1] >= maze_size[1]
                or end_p == (0, 0)
                or cur_p == (maze_size[0] - 1, maze_size[1] - 1)
                or cur_p[0] == end_p[0]
                or cur_p[1] == end_p[1]
            ):
                return False
            ...
            if cur_horizontal:
                points.extend(
                    [
                        x + cur_p[1] * maze_size[1]
                        for x in range(cur_p[0], end_p[0], 1 if cur_p[0] < end_p[0] else -1)
                    ]
                )
                points.extend(
                    [
                        end_p[0] + y * maze_size[1]
                        for y in range(cur_p[1], end_p[1], 1 if cur_p[1] < end_p[1] else -1)
                    ]
                )
            else:
                points.extend(
                    [
                        cur_p[0] + y * maze_size[1]
                        for y in range(cur_p[1], end_p[1], 1 if cur_p[1] < end_p[1] else -1)
                    ]
                )
                points.extend(
                    [
                        x + end_p[1] * maze_size[1]
                        for x in range(cur_p[0], end_p[0], 1 if cur_p[0] < end_p[0] else -1)
                    ]
                )
            for p in points:
