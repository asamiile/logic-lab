...
class MazeGenomeDecoder:
    ...
    def decode(self, genome, config, save=None, return_env=True):
        ...
        path_length = self.track_pathway(
            maze_size, path_genes, pathway_map, horizontal_wall_map, vertical_wall_map
        )
        ...
        subregion_num = self.map_walls_of_regions(
            wall_genes, regions, maze_size, pathway_map, horizontal_wall_map, vertical_wall_map
        )
        ...
        if return_env:
            ...
            exit_point = (
                maze_size[0] * self.maze_scaler - self.maze_scaler // 2,
    ...
    def divide_maze(self, maze_size, path_map):
        ...
        for y, x in zip(*np.where(region_map == 0)):
            ...
            if np.all(cut_out == 0):
                ...
                x_width = (
                    cut_out.shape[1]
                    if np.all(cut_out[0, :] == 0)
                    else np.where(cut_out[0, :] != 0)[0][0]
                )
                x_height = (
                    cut_out.shape[0]
                    if np.all(cut_out[:, :x_width] == 0)
                    else np.where(np.any(cut_out[:, :x_width] != 0, axis=1))[0][0]
                )
                # height-first square
                y_height = (
                    cut_out.shape[0]
                ...
                y_width = (
                    cut_out.shape[1]
                    if np.all(cut_out[:y_height, :] == 0)
                    else np.where(np.any(cut_out[:y_height, :] != 0, axis=0))[0][0]
                )
        ...
        while subregion_queue:
            ...
            if horizontal:
                ...
                if passage_x + 1 < r_width:
                    h_wall_map[
                        r_start[1] + wall_y, r_start[0] + passage_x + 1 : r_start[0] + r_width
                    ] = True
                ...
                if passage_y + 1 < r_height:
                    v_wall_map[
                        r_start[1] + passage_y + 1 : r_start[1] + r_height, r_start[0] + wall_x
                    ] = True
    ...
    def make_entrance(
        self, h_point, v_point, start_p, end_p, maze_size, path_map, h_wall_map, v_wall_map
    ):
        step = 1
        # search pathway
        ...
        for h_i in range(maze_size[1] + 1):
            ...
            for now_i in no_walls:
                if prev_i < now_i:
                    walls.append(
                        (
                            prev_i * self.maze_scaler,
                            h_i * self.maze_scaler,  # start point
                            now_i * self.maze_scaler,
                            h_i * self.maze_scaler,
                        )  # end point
                    )
                prev_i = now_i + 1
        ...
        for w_i in range(maze_size[0] + 1):
            ...
            for now_i in no_walls:
                if prev_i < now_i:
                    walls.append(
                        (
                            w_i * self.maze_scaler,
                            prev_i * self.maze_scaler,  # start point
                            w_i * self.maze_scaler,
                            now_i * self.maze_scaler,
                        )  # end point
                    )
