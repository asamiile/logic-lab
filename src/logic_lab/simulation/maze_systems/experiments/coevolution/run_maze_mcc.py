...
ENV_DIR = os.path.join(PROJ_DIR, "environment", "maze")
...
class MazeReporter(mcc.BaseReporter):
    def end_generation(self, config, agent_genomes, maze_genomes):
        ...
        nn_conns = [
            len([1 for conn_gene in agent_genome.connections.values() if conn_gene.enabled])
            for agent_genome in agent_genomes.values()
        ]

        print("   statistics         min      ave      max")
        print("                    =======  =======  =======")
        print(
            f"maze area        :  {np.min(maze_areas): =7.1f}  {np.mean(maze_areas): =7.1f}  {np.max(maze_areas): =7.1f}"
        )
        print(
            f"maze junctures   :  {np.min(maze_junctures): =7.1f}  {np.mean(maze_junctures): =7.1f}  {np.max(maze_junctures): =7.1f}"
        )
        print(
            f"maze path length :  {np.min(maze_path_length): =7.1f}  {np.mean(maze_path_length): =7.1f}  {np.max(maze_path_length): =7.1f}"
        )
        print(
            f"nn connections   :  {np.min(nn_conns): =7.1f}  {np.mean(nn_conns): =7.1f}  {np.max(nn_conns): =7.1f}"
        )
...
def main():
    ...
    config = mcc.make_config(
        mcc.DefaultGenome, MazeGenome, config_file, custom_config=custom_config
    )
    ...
    MazeDecoder = MazeGenomeDecoder(
        config.genome2_config, maze_kwargs=maze_config, agent_kwargs=agent_config
    )
    ...
    assert os.path.exists(agent_bootstrap_file) or not os.path.exists(
        maze_bootstrap_file
