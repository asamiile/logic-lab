...
def make_figure(
    maze_key,
    maze_file,
    maze_decode_function,
    maze_config,
    agent_file,
    agent_decode_function,
    agent_config,
    figure_file,
    colorbar=False,
):
...
def main():
    ...
    if not os.path.exists(maze_history_file):
        print(f"Error: {maze_history_file} not found. Run run_maze_mcc.py first.")
        return

    with open(maze_history_file, newline="") as f:
        reader = csv.reader(f)
        histories = [row for row in list(reader)[1:] if len(row) >= 4]

        if not histories:
            print(f"Error: {maze_history_file} is empty. Wait for run_maze_mcc.py to complete.")
            return

        condition = lambda z: z >= args.start_generation and z < args.end_generation
        pairs = [
            (int(hist[1]), ast.literal_eval(hist[3])[0])
            for hist in histories
            if condition(int(hist[0]))
        ]

        if not pairs:
            print(f"No data found in generations {args.start_generation} to {args.end_generation}")
            return
    ...
    if not args.no_multi:
        ...
        for maze_key, agent_key in pairs:
            ...
            func_args = (
                ...
                maze_genome_file,
                maze_decode_function,
                config.genome2_config,
                agent_genome_file,
                agent_decode_function,
                config.genome1_config,
                figure_file,
            )
            func_kwargs = {
