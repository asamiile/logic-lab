...
class ScoreReporter(ns_neat.BaseReporter):
    ...
    def post_evaluate(self, config, population, species, best_genome):
        ...
        seconds = elapsed % 60
        print(
            f"\rgeneration: {self.generation :3}  best: {self.best: =.3f}  elapsed: {hours:0=2}:{minutes:0=2}:{seconds:0=4.1f}",
            end="",
        )
...
def main():
    ...
    MazeDecoder = MazeGenomeDecoder(
        mcc_config.genome2_config, maze_kwargs=maze_config, agent_kwargs=agent_config
    )
    ...
    while len(maze_genomes) < args.maze_num:
        ...
        maze_env, timesteps = MazeDecoder.decode(
            maze_genome, mcc_config, save=os.path.join(save_path, f"maze{maze_genome.key+1}.jpg")
        )
        ...
        while len(agent_genomes_tmp) < perMaze:
            ...
            if agent_genome.score >= 1.0:
                ...
                agent_genome.success_keys = [maze_genome.key]
