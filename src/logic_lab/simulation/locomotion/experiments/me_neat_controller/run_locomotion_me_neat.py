...
def main():
    ...
    config_content = config_content.replace(
        "offspring_size        = 50", f"offspring_size        = {args.batch_size}"
    )
    ...
    bd_dictionary = {
        "forward_speed": BD.ForwardSpeed(
            name="forward_speed", value_range=[-2.0, 3.0], resolution=20
        ),
        "lateral_stability": BD.LateralStability(
            name="lateral_stability", value_range=[0.0, 1.0], resolution=20
        ),
        "body_tilt": BD.BodyTilt(
            name="body_tilt", value_range=[-np.pi / 2, np.pi / 2], resolution=20
        ),
        "joint_activity": BD.JointActivity(
            name="joint_activity", value_range=[0.0, 1.0], resolution=20
        ),
        "step_frequency": BD.StepFrequency(
            name="step_frequency", value_range=[0.0, 5.0], resolution=20
        ),
    }
    ...
    reporters = [
        ...
        me_neat.BDDrawer(
            save_path, bd_dictionary[bd_axis[0]], bd_dictionary[bd_axis[1]], no_plot=args.no_plot
        ),
    ]
