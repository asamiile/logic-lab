...
def transfer_step(env_kwargs, config, params, obs_rms):
    ...
    policy = Policy(
        envs.observation_space, envs.action_space, init_log_std=config.init_log_std, device="cpu"
    )
...
class OptimizerPPO:
    ...
    def set_env_info(self, make_env_kwargs, config):
        ...
        self.make_env_kwargs_transfer = dict(
            **make_env_kwargs, num_processes=config.num_processes, vecnormalize=True, subproc=False
        )
        self.make_env_kwargs_eval = dict(
            **make_env_kwargs, num_processes=1, vecnormalize=True, subproc=False
        )
    ...
    def get_evaluate(self, config, core=None):
        ...
        func = evaluate
        kwargs = {"env_kwargs": self.make_env_kwargs_eval, "params": params, "obs_rms": obs_rms}
        return func, kwargs
