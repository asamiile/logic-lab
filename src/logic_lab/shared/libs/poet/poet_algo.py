...
class POET:
    ...
    def add_niche(self, key, niche):
        ...
        with open(self.history_file, "a") as f:
            writer = csv.DictWriter(f, fieldnames=self.history_header)
            items = {"iteration": self.iteration, "key": key, "parent": niche.parent}
    ...
    def evaluate_niches(self, niches, imigrant_cores=None):
        for key, niche in niches.items():
            niche.start_evaluate(
                self.pool, self.env_config, self.opt_config, imigrant_cores=imigrant_cores
            )

        for key, niche in niches.items():
            reward = niche.end_evaluate(
                self.env_config, self.opt_config, imigrant_cores=imigrant_cores
            )
    ...
    def develop_niches(self):
        ...
        for key, niche in self.niches.items():
            print(
                f" {key: =6} :   {niche.steps: =5}   {niche.reward: =+8.2f}   {niche.best_reward: =+8.2f}"
            )
        print()
    ...
    def transfer(self, reciever_niches, child=False):
        ...
        for reciever_key, reciever_niche in reciever_niches.items():
            ...
            accepted_cores = {
                accepted_key: imigrant_niche_cores[accepted_key] for accepted_key in accepted_keys
            }
            reciever_niche.start_step(
                self.pool, self.env_config, self.opt_config, imigrant_cores=accepted_cores
            )
            adapted_cores = reciever_niche.end_step(
                self.env_config, self.opt_config, imigrant_cores=accepted_cores
            )
        ...
        for reciever_key, adapted_cores in adapts.items():
            ...
            reciever_niche.start_evaluate(
                self.pool,
                self.env_config,
                self.opt_config,
                imigrant_cores=adapted_cores,
                invasion=True,
            )
        ...
        for reciever_key, reciever_niche in reciever_niches.items():
            ...
            if reciever_key in adapts:
                ...
                invader = reciever_niche.end_evaluate(
                    self.env_config,
                    self.opt_config,
                    imigrant_cores=adapted_cores,
                    invasion=True,
                ...
                if len(invader) == 1:
                    ...
                    print(
                        f" {reciever_key: =6} :           yes   {invader_key: =6}   {reward: =+8.2f}",
                        end=print_end,
                    )
    ...
    def get_novelty_score(self, niche, archives):
        ...
        transferred_rewards = np.clip(
            transferred_rewards, self.clip_reward_lower, self.clip_reward_upper
        )

        distances = []
        for _, archive_niche in archives.items():
            archive_transferred_rewards = archive_niche.get_transferred_rewards(keys)
            archive_transferred_rewards = np.clip(
                archive_transferred_rewards, self.clip_reward_lower, self.clip_reward_upper
            )
    ...
    def get_child_niches(self, reproduction_num):
        ...
        parent_niches = [
            niche for niche in self.niches.values() if niche.reward > self.repro_threshold
        ]
        ...
        for _ in range(reproduction_num):
            ...
            child_niche = parent_niche.reproduce(
                child_key, self.iteration, self.env_config, self.opt_config
            )
        ...
        child_niches_pass = {
            key: niche for key, niche in child_niche_candidates.items() if self.pass_mc(niche)
        }
    ...
    def end_iteration(self):
        save_core = (
            self.save_core_interval > 0 and (self.iteration + 1) % self.save_core_interval == 0
        )
        for key, niche in self.niches.items():
