...
class Niche:
    ...
    def end_step(self, env_config, opt_config, imigrant_cores=None):
        ...
        if imigrant_cores is None:
            ...
            for imigrant_key, imigrant_core in imigrant_cores.items():
                ...
                cores[imigrant_key] = self.optimizer.step(
                    results, opt_config, core=imigrant_core, update=False
                )
        ...
        if imigrant_cores is None:
            ...
            for imigrant_key, imigrant_core in imigrant_cores.items():
                if not invasion and imigrant_key in self.transferred_rewards:
                    self.processes[imigrant_key] = pool.apply_async(
                        self.transferred_rewards.get, args=(imigrant_key,)
                    )
    ...
    def get_accepted_keys(self, keys):
        ...
        accepted_keys = [
            key for key in keys if key != self.key and self.transferred_rewards[key] > base_reward
        ]
