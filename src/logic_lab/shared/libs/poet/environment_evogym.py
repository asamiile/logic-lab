...
class EvogymTerrainDecoder(neat_cppn.BaseCPPNDecoder):
    def __init__(self, max_width, first_platform=10):
        ...
        self.input_keys = ["x", "platform", "voxel", "wall"]

        self.output_keys = {
            "platform": ["flat", "height", "width"],
            "voxel": ["rigid", "soft", "empty"],
        }
    ...
    def decode(self, genome, config, terrain_param):
        ...
        neighbors = {
            (x + i, y): (
                ...
                if i == 0
                    ...
                    [(x + i - 1, y)]
                    if i == self.first_platform - 1
                    else [(x + i - 1, y), (x + i + 1, y)]
                )
            )
            for i in range(self.first_platform)
        }
        ...
        while x < self.max_width:
            ...
            rigid, soft, empty = (
                (rigid + 1) * terrain_param.rigid_bias,
                (soft + 1) * terrain_param.soft_bias,
                (empty + 1) * terrain_param.empty_bias,
            )
            ...
            height = (
                round(height * flat**2 * terrain_param.max_up_step)
                if height > 0
                else round(height * flat**2 * terrain_param.max_down_step)
            )
            width = width / 2 + 0.5
            width = (
                int(width * terrain_param.max_empty_width) + 1
                if voxel_type == 0
                else (
                    int(width * (terrain_param.max_soft_width - 1)) + 2
                    if voxel_type == 2
