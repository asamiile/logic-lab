...
def make_config(config_file, extra_info=None, custom_config=None):
    config = MEConfig(
        neat_cppn.DefaultGenome, config_file, extra_info=extra_info, custom_config=custom_config
    )
    return config
...
class MEConfig:
        ...
        for p in self.__params:
            if p.default is None:
                ...
                try:
                    ...
                    warnings.warn(
                        f"Using default {p.default!r} for '{p.name!s}'", DeprecationWarning
                    )
        ...
        if unknown_list:
            if len(unknown_list) > 1:
                raise UnknownConfigItemError(
                    "Unknown (section 'ME-NEAT') configuration items:\n" + "\n\t".join(unknown_list)
                )
            raise UnknownConfigItemError(
                f"Unknown (section 'ME-NEAT') configuration item {unknown_list[0]!s}"
            )
    ...
    def save(self, filename):
        with open(filename, "w") as f:
            f.write(
                "# The `ME-NEAT` section specifies parameters particular to the ME-NEAT algorithm\n"
            )
