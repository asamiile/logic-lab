...
def make_config(genome1, genome2, config_file, extra_info=None, custom_config=None):
    config = MCCConfig(
        genome1, genome2, config_file, extra_info=extra_info, custom_config=custom_config
    )
    return config
...
class MCCConfig:
        ...
        if unknown_list:
            if len(unknown_list) > 1:
                raise UnknownConfigItemError(
                    "Unknown (section 'MCC') configuration items:\n" + "\n\t".join(unknown_list)
                )
            raise UnknownConfigItemError(
                f"Unknown (section 'MCC') configuration item {unknown_list[0]!s}"
            )
    ...
    def save(self, filename):
        with open(filename, "w") as f:
            ...
            f.write(f"\n[{self.genome1_type.__name__}]\n")
            self.genome1_type.write_config(f, self.genome1_config)

            f.write(f"\n[{self.genome2_type.__name__}]\n")
