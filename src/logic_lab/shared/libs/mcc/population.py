...
class Population:
    ...
    def run(self, evaluate_function, n=None):
        ...
        while k < n:
            ...
            genome1_offsprings = self.genome1_reproduction.create_offsprings(
                self.genome1_pop, self.config.genome1_offspring_size, self.generation
            )
            genome2_offsprings = self.genome2_reproduction.create_offsprings(
                self.genome2_pop, self.config.genome2_offspring_size, self.generation
            )
            ...
            genome1_survivors = {
                key: genome
                for key, genome in genome1_offsprings.items()
                if genome.fitness >= self.config.genome1_criterion
            }
            genome2_survivors = {
                key: genome
                for key, genome in genome2_offsprings.items()
                if genome.fitness >= self.config.genome2_criterion
            }

            self.reporters.post_evaluate(self.config, genome1_survivors, genome2_survivors)

            self.genome1_pop = self.update_pop(
                self.genome1_pop,
                genome1_survivors,
                self.config.genome1_pop_size,
                self.config.genome1_config,
            )
            self.genome2_pop = self.update_pop(
                self.genome2_pop,
                genome2_survivors,
                self.config.genome2_pop_size,
                self.config.genome2_config,
            )
