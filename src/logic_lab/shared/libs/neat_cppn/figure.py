...
def make_species(expt_path):
    if not HAS_PANDAS or not HAS_MATPLOTLIB:
        print("Warning: pandas and matplotlib are required for make_species function")
        return
    ...
    for key in data["species"].unique():
        ...
        pop_history = np.array(
            [
                pop_history.get(gen, 0) / generation_pop_size[gen]
    ...
    for i in range(len(order)):
        ...
        if i > 0:
            base_y[key] = base_y[prev_key] + max(
                0.2, np.max(np.where((prev_pop > 0) & (pop > 0), prev_pop, 0)) + 0.1
            )
