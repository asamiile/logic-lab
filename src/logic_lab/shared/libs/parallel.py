...
class EvaluatorParallel:
    ...
    def evaluate(self, genomes, config, generation):
        ...
        if self.parallel:
            phenomes = {
                key: self.decode_function(genome, config.genome_config)
...
class MCCEvaluatorParallel:
    def __init__(
        self, num_workers, evaluate_function, decode_function1, decode_function2, timeout=None
    ):
        self.num_workers = num_workers
        self.evaluate_function = evaluate_function
    ...
    def evaluate(
        ...
        offsprings2_phenome = {
            key2: self.decode_function2(genome2, config.genome2_config)
            for key2, genome2 in offsprings2_genome.items()
        }

        population1_phenome = {
            key1: self.decode_function1(genome1, config.genome1_config)
            for key1, genome1 in population1_genome.items()
        }
        population2_phenome = {
            key2: self.decode_function2(genome2, config.genome2_config)
            for key2, genome2 in population2_genome.items()
        }
        ...
        population1_count = {
            key1: self.manager.Value("i", len(genome1.success_keys))
            for key1, genome1 in population1_genome.items()
        }
        population2_count = {
            key2: self.manager.Value("i", len(genome2.success_keys))
            for key2, genome2 in population2_genome.items()
        }

        # evaluate genome1
        self.evalute_one_side(
            offsprings1_genome,
            offsprings1_phenome,
            offsprings1_count,
            population2_genome,
    ...
    def evalute_one_side(
        self, genomes1, phenomes1, counts1, genomes2, phenomes2, counts2, config, generation
    ):
        jobs = {}
    ...
    @staticmethod
    def conditioned_evaluation(
        phenome1, achieve1, phenome2, achieve2, config, generation, evaluate_function
    ):
        count_up = False
        ...
        if (config.genome1_limit > 0 and achieve1.value >= config.genome1_limit) or (
            ...
            return count_up
        if (
            achieve1.value >= config.genome1_criterion
            and achieve2.value >= config.genome2_criterion
        ):
            return count_up
        ...
        if success:
            if (
                achieve1.value < config.genome1_criterion
                and (config.genome2_limit == 0 or achieve2.value < config.genome2_limit)
            ) or (
